    ...
    def _current_step_finished_req_ids(
        ...
        if eos_id is not None:
            try:
                ...
                pass
        # Qwen3-TTS uses the pipeline stop token to terminate codec
        # generation. Some checkpoints do not expose the same value through
        # _codec_eos_token_id, so include the known stop token used by the
        # pipeline to make in-process tail flush run on the terminal step.
        if getattr(self.model, "tts_inprocess_fusion_enable", False):
            eos_ids.add(2150)
        if not eos_ids:
            return set()
        ...
        for idx, rid in enumerate(req_ids_output_copy):
            ...
            if isinstance(token_ids, torch.Tensor):
                ...
                sampled = [token_ids]
            if eos_ids.intersection({int(t) for t in sampled if t is not None}):
                finished.add(rid)
        return finished

    def _current_step_terminal_req_ids(
        self,
        req_ids_output_copy: list[str],
        valid_sampled_token_ids: Any,
    ) -> set[str]:
        finished = self._current_step_finished_req_ids(
            req_ids_output_copy,
    ...
    def sample_tokens(
        ...
        if needs_pooler_payload:
            ...
            if callable(decode_inprocess_tts):
                ...
                current_finished_req_ids = self._current_step_terminal_req_ids(
                    req_ids_output_copy,
                    valid_sampled_token_ids,
                )
                if current_finished_req_ids:
                    logger.info(
                        "[Qwen3TTS][inprocess_fusion] current_step_terminal req_ids=%s",
            ...
            if tail_flush_output is not None and tail_flush_output.pooler_output is not None:
                merged_tail_req_ids: list[str] = []
                for tail_pos, tail_rid in enumerate(tail_flush_output.req_ids):
                    if tail_rid not in req_id_to_index_output_copy:
                        continue
                    if tail_pos >= len(tail_flush_output.pooler_output):
                        continue
                    idx = req_id_to_index_output_copy[tail_rid]
                    tail_payload = tail_flush_output.pooler_output[tail_pos]
                    if isinstance(tail_payload, dict):
                        pooler_output[idx].update(tail_payload)
                        merged_tail_req_ids.append(tail_rid)
                if merged_tail_req_ids:
                    logger.info(
                        "[Qwen3TTS][inprocess_fusion] merged tail_flush into scheduled output req_ids=%s",
                        sorted(merged_tail_req_ids),
                    )
                else:
                    logger.warning(
                        "[Qwen3TTS][inprocess_fusion] tail_flush could not merge into scheduled output req_ids=%s",
                        sorted(tail_flush_output.req_ids),
                    )
